...
// --- LOGOUT ---
function logout() {
  token=''; userRole=''; selectedLecturerId=null;
...
async function showLecturerDashboard() {
  document.getElementById('loginDiv').classList.add('hidden'); document.getElementById('registerDiv').classList.add('hidden');
  document.getElementById('lecturerDiv').classList.remove('hidden');

  const res = await fetch(API_BASE+'/lecturer/today-classes',{headers:{'Authorization':'Bearer '+token}});
  const classes = await res.json();
  const tbody = document.getElementById('lecturerTable').querySelector('tbody');
  tbody.innerHTML='';
  classes.forEach(cls=>{
    const tr=document.createElement('tr');
    tr.innerHTML=`<td>${cls.course_name}</td>
...
async function showSupervisorDashboard() {
  ...
  const res = await fetch(API_BASE+'/supervisor/lecturers',{headers:{'Authorization':'Bearer '+token}});
  const lecturers = await res.json();
  const select = document.getElementById('lecturerSelect');
  select.innerHTML='';
  lecturers.forEach(l=>{ const opt=document.createElement('option'); opt.value=l.lecturer_id; opt.textContent=l.name; select.appendChild(opt); });
  selectedLecturerId = select.value;
  loadClassRecords();
}

async function loadClassRecords() {
  selectedLecturerId=document.getElementById('lecturerSelect').value;
  const res = await fetch(`${API_BASE}/supervisor/class-records/${selectedLecturerId}`,{headers:{'Authorization':'Bearer '+token}});
  const records = await res.json();
  const tbody=document.getElementById('supervisorTable').querySelector('tbody');
  tbody.innerHTML='';
  records.forEach(r=>{
    const tr=document.createElement('tr');
    tr.innerHTML=`<td>${r.course_name}</td>
      <td>${r.date}</td><td>${r.actual_start}</td><td>${r.actual_end}</td>
      <td>${r.notes}</td><td>${r.status}</td>
      <td>${r.status==='pending'?`<button onclick="approveRecord(${r.record_id})">Approve</button>
      <button onclick="rejectRecord(${r.record_id})">Reject</button>`:''}</td>`;
    tbody.appendChild(tr);
  });
}
...
async function approveRecord(recordId){
  ...
  await fetch(API_BASE+'/supervisor/approve',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({record_id:recordId,status:'approved',remarks})});
  loadClassRecords();
}

async function rejectRecord(recordId){
  const remarks=prompt('Add remarks for rejection:');
  await fetch(API_BASE+'/supervisor/approve',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({record_id:recordId,status:'rejected',remarks})});
  loadClassRecords();
}

// --- MONTHLY REPORT ---
async function showMonthlyReport(){
  if(!selectedLecturerId) return alert('Select a lecturer first');
  const res = await fetch(`${API_BASE}/supervisor/class-records/${selectedLecturerId}`,{headers:{'Authorization':'Bearer '+token}});
  const records = await res.json();
  ...
  records.forEach(r=>{
    const tr=document.createElement('tr');
    tr.innerHTML=`<td>${r.course_name}</td><td>${r.date}</td><td>${r.actual_start}</td><td>${r.actual_end}</td>
      <td>${r.notes}</td><td>${r.status}</td><td>${r.remarks||''}</td>`;
    tbody.appendChild(tr);
  });
}

function printReport(){ window.print(); }
function hideMonthlyReport(){ document.getElementById('monthlyReportDiv').classList.add('hidden'); }
